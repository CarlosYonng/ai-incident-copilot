...
- 系统创建 Incident，调用 MCP 诊断工具、查询 mock metrics、检索 Runbook、生成处置方案，人工标记线下已执行后观察恢复并生成复盘。
...
人工对处置方案的决策，包括批准、驳回、要求补充证据、升级 SRE、标记线下已执行、标记无效、关闭 Incident。

## 6. Workflow 节点

1. `AlertReceiverNode`：接收告警并创建 Incident。
2. `MetricsCollectorNode`：查询 mock metrics。
...
10. `ObserveRecoveryNode`：观察模拟指标恢复。
